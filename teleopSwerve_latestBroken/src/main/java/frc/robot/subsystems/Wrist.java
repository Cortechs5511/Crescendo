...
public class Wrist extends SubsystemBase {
    ...
    public void setIntakePosition() {
        double motorOutput;
        if (getRawPosition() > 0.5) {
            motorOutput = wristPIDController.calculate(getRawPosition(), WristConstants.WRIST_INTAKE_POSITION);
        }
        else {
             motorOutput = -1 * wristPIDController.calculate(getRawPosition(), WristConstants.WRIST_INTAKE_POSITION);
        }
        setPower(motorOutput);
        SmartDashboard.putNumber("Wrist/PID Output", motorOutput);
    }
